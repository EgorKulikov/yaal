...
public class Knight {
	public void solve(int testNumber, InputReader in, OutputWriter out) {
		...
		for (int i = 0; i < edgeCount; i++) {
			...
			int to = in.readInt() - 1;
			int cost = in.readInt();
			graph.add(new WeightedFlowEdge<Integer>(2 * from + 1, 2 * to, cost, 2));
			graph.add(new WeightedFlowEdge<Integer>(2 * to + 1, 2 * from, cost, 2));
		}
		...
		for (int i = 0; i < vertexCount; i++)
			graph.add(new WeightedFlowEdge<Integer>(2 * i, 2 * i + 1, 0, isGuarded[i] ? 1 : 2));
