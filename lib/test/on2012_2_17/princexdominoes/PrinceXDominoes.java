package on2012_2_17.princexdominoes;
	...
	public int play(String[] dominoes) {
		...
		for (int i = 0; i < count + 2; i++) {
			for (int j = 0; j < count + 2; j++) {
				if (edgeCount[i][j] != 0)
					graph.add(new WeightedFlowEdge<Integer>(i, j, Math.max(i, j) >= count ? 0 : -1, edgeCount[i][j]));
			}
		}
		Edge<Integer>[] last = new Edge[count + 2];
		Edge<Integer>[] next = new Edge[count + 2];
			...
			for (int i = 0; i < 2 * count; i++) {
				for (int j = 0; j < count; j++) {
					if (distance[j] == Long.MAX_VALUE)
						continue;
					for (Edge<Integer> edge : graph.getIncident(j)) {
