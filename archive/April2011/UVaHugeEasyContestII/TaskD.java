...
public class TaskD implements Solver {
	...
	private BigInteger calculate(ArrayWrapper<String> expression) {
		if (expression.size() == 1)
			return new BigInteger(expression.get(0), 2);
		for (int i = expression.size() - 1; i >= 0; i--) {
			String token = expression.get(i);
			if (token.equals("or"))
				return calculate(expression.subArray(0, i)).or(calculate(expression.subArray(i + 1)));
			if (token.equals("xor"))
				return calculate(expression.subArray(0, i)).xor(calculate(expression.subArray(i + 1)));
			if (token.equals("and"))
				return calculate(expression.subArray(0, i)).and(calculate(expression.subArray(i + 1)));
		}
		BigInteger result = calculate(expression.subArray(1));
