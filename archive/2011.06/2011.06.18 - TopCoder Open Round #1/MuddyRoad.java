public class MuddyRoad {
	public double getExpectedValue(int[] road) {
		int partCount = road.length;
		double[][] probability = new double[partCount][partCount + 1];
		probability[0][0] = 1 - road[0] / 100.;
		probability[0][1] = 1 - probability[0][0];
		for (int i = 0; i < partCount - 1; i++) {
			for (int j = 0; j < partCount; j++) {
				if (probability[i][j] == 0)
					continue;
				if (i == partCount - 2) {
					probability[i + 1][j + 1] += probability[i][j] * road[i + 1] / 100.;
					probability[i + 1][j] += probability[i][j] * (1 - road[i + 1] / 100.);
				} else {
					probability[i + 2][j + 1] += probability[i][j] * road[i + 1] * road[i + 2] / 10000.;
					probability[i + 2][j] += probability[i][j] * road[i + 1] * (1 - road[i + 2] / 100.) / 100.;
					probability[i + 1][j] += probability[i][j] * (1 - road[i + 1] / 100.);
				}
			}
		}
		...
		for (int i = 1; i <= partCount; i++)
			result += probability[partCount - 1][i] * i;
		return result;
	}
...
class MuddyRoadHarness {
	...
	static int runTestCase(int casenum) {
		...
		case 0: {
			...
			return verifyCase(casenum, expected__, new MuddyRoad().getExpectedValue(road));
		}
		case 1: {
