	...
	public void solve(int testNumber, net.egork.utils.old.io.old.InputReader in, PrintWriter out) {
		...
		for (int i = 0; i < queries; i++) {
			...
			if ("sum".equals(command))
				out.println(set.result());
			else if ("add".equals(command))
				set.add(in.readInt());
			else
				set.remove(in.readInt());
		}
	}

private static class TreapSet<K> implements NavigableSet<K> {
	...
	protected class Node {
		...
		protected void updateSize() {
			...
			for (int i = 0; i < 5; i++) {
				result[i] = left.result[i] + right.result[((i - left.size - 1) % 5 + 5) % 5] + (left.size % 5 == i ? value(key) : 0);
			}
		}
	...
	private static int value(Object key) {
